...
func progressUpdate (steps : Int, goal : Int)
{
    ...
    else if steps > 1001 && steps < goal / 2
    {
        print("You're almost halfway there!")
    }
    else if Double(steps) > 5001 && Double(steps) < Double(goal) * 0.9
    {
        print("You're over halfway there!")
    }
    else if Double(steps) > Double(goal) * 0.9 && steps < goal
    {
...
progressUpdate(steps: 4, goal: 10000)
progressUpdate(steps: 1003, goal: 10000)
progressUpdate(steps: 99999999, goal: 10000)

/*:
 Your fitness tracking app is going to help runners stay on pace to reach their goals. Write a function called pacing that takes four `Double` parameters called `currentDistance`, `totalDistance`, `currentTime`, and `goalTime`. Your function should calculate whether or not the user is on pace to hit or beat `goalTime`. If yes, print "Keep it up!", otherwise print "You've got to push it just a bit harder!"
 */
func pacing (currentDistace : Double, totalDistance : Double, currentTime : Double, goalTime: Double)
{
    let remainsD = currentDistace / totalDistance
    let remainsT = currentTime / goalTime
    if remainsD > remainsT
    {
        print ("Keep it up!")
    }
    else
...
pacing(currentDistace: 20, totalDistance: 100, currentTime: 3, goalTime: 12)
pacing(currentDistace: 60, totalDistance: 100, currentTime: 3, goalTime: 12)
